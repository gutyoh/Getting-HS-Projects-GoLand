...
_A VERY IMPORTANT DETAIL is that for some reason the `hs-test-python` package included in the Project Files has an error, and doesn't properly run the tests.
...
The new version of the `hs-test-python` package will now have been installed, and you'll be able to properly run all _tests.py_ file of each project stage! Good job troubleshooting!